...
(1) Identify subtype gene loci in input genomes using BLAST \citep{Camacho2009}. Hits that do not align over 95\% or have under 90\% sequence identity with reference sequence are discarded.
...
Phylotyper assumes that the provided training sequence are 1) homologous, specifically, they are suitable for alignment and phylogenetic reconstruction 2) the sequence phylogeny is correlated with the subtype distribution and 3) the set is representative of the range of sequences that make up a subtype.
...
Specifically, each new subtype is subject to two tests in the \texttt{new} pipeline.
Results are reported to the user.
The first test checks the the distribution of inter-patristic phylogenetic distances between the instances of the same subtype is both smaller and distinct from non-identical subtypes.
...
A second check computes the accuracy measure; F-score, through a leave-one-out cross-validation.
This validation uses each sequence in the training set as a test input (the test sequence is removed from the tree and then used as input).
Results are recorded. 
Metrics like precision, recall and F-score rapidly erode as the phylogenetic grouping is decoupled from the subtypes.
These checks will verify that the sequences phylogenetic grouping is predictive of the subtype subtype.
There is no check that can verify the training set covers all subtypes; this is left up to the user.
However, as we show in the Results section, Phylotyper is comparatively more robust than other methods as it generates fewer Type-I errors when encountering novel alleles or subtypes not present in the training set.
...
All new or updated subtype schemes added in Phylotyper are subject to a leave-one-out cross-validation test as described earlier.  
...
For example SerotypeFinder uses BLAST to find the top matches based on sequence similarity to O-antigen processing genes for \textid{in silico} O-typing, and the flagellin genes for H-typing \textit{E. coli} genes.  
