##########################################################
## File: phylogenetic_signal.R
##
##  Computes phylogenetic signal for subtype tree
##
## Slatkin & Madison or lambda transformation methods
## 
##
##
##
## author: Matt Whiteside
## copyright: Copyright Government of Canada 2012-2015. Funded by the Government of Canada Genomics Research and Development Initiative
## license: ASL
## version: 2.0
## maintainer: Matt Whiteside
## email: mdwhitesi@gmail.com
##
##########################################################

http://evolve.zoo.ox.ac.uk/Evolve/BaTS.html - estimation of association index AI, Fitch parsimony score PS and maximum exclusive single-state clade size MC
http://finzi.psych.upenn.edu/library/pez/html/phy.signal.html - lambda
http://www.carlboettiger.info/2013/10/11/is-it-time-to-retire-pagels-lambda.html - lambda issues
http://artax.karlin.mff.cuni.cz/r-help/library/geiger/html/deltaTree.html - lambda transformation
http://www.eve.ucdavis.edu/~wainwrightlab/Roi/Site/Teaching_files/Intro2Phylo_S4.R - lambda approach
https://github.com/juliema/publications/blob/master/BrueeliaMS/Maddison.Slatkin.R - Slatkin & Madison approach
http://cpansearch.perl.org/src/CJFIELDS/BioPerl-1.6.924/Bio/Tree/Statistics.pm - AI, PS and MC tree stats





